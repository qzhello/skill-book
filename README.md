...
- **GitHub 备份 / 恢复**：一键把用户级 `~/.claude/skills` 与 `~/.codex/skills` 备份到你自己的 GitHub 私有仓库，并能从备份恢复（被覆盖的本地目录先移废纸篓）。token 经环境变量传给 git（不入 URL / 配置 / 日志），仅存本机 `~/.skillbook/backup.json`（0600）。
